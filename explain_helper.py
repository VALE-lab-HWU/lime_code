...
def get_data_per_classification(data, index_cl):
    return [(data[index_cl[i]], i) for i in index_cl]


def save_histogram_per_data(datas, title, n_range=None, n_bins=218):
    for data, i in datas:
        save_histogram(data, title+'_'+i, n_range, n_bins)


def save_all_histogram_all_data(data, index_cl, title):
    title = 'all'
    n_range = None
    n_bins = 218
    save_histogram(data, title+'_full', n_range, n_bins)
    data_cl = get_data_per_classification(data, index_cl)
    save_histogram_per_data(data_cl, title, n_range, n_bins)
    data = remove_over_represented_data(data)
    title = 'corrected'
    save_histogram(data, title+'_full', n_range, n_bins)
    data_cl = [(remove_over_represented_data(d), i)for d, i in data_cl]
    save_histogram_per_data(data_cl, title, n_range, n_bins)
